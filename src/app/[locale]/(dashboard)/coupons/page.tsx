...
import { Tag, Percent, BadgeCheck, CalendarClock, TicketX, Plus } from "lucide-react";
import type { Locale } from "@/i18n/routing";
import { Link } from "@/i18n/navigation";
import { requireModuleAccess } from "@/lib/auth/guard";
import { listCoupons, getCouponStats } from "@/lib/coupons/queries";
...
export default async function CouponsPage({
  ...
  const statusHref = (s: string) => `/coupons?${new URLSearchParams({ status: s }).toString()}`;

  const statusOptions = ["ACTIVE", "SCHEDULED", "EXPIRED", "DISABLED"] as const;

  const cards = [
    { icon: Tag, label: t("stats.total"), value: formatNumber(stats.total, locale), tone: "primary" as const },
    { icon: BadgeCheck, label: t("stats.active"), value: formatNumber(stats.active, locale), tone: "success" as const },
    {
      icon: CalendarClock,
      label: t("stats.scheduled"),
      value: formatNumber(stats.scheduled, locale),
      tone: "info" as const,
    },
    {
      icon: Percent,
      label: t("stats.redemptions"),
      value: formatNumber(stats.totalRedemptions, locale),
      meta: t("stats.redemptionsHint", { count: stats.totalDiscountedOrders }),
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <FilterChip href="/coupons" active={!status} label={t("filter.all")} />
        {statusOptions.map((s) => (
            ...
            <table className="w-full min-w-220 border-collapse text-sm">
              <thead>
                <tr className="border-border text-muted-foreground border-b text-xs font-semibold tracking-wider uppercase">
                  ...
                  <th className="px-4 py-3 text-start font-semibold">{t("table.discount")}</th>
                  <th className="px-4 py-3 text-start font-semibold">{t("table.minOrder")}</th>
                  <th className="px-4 py-3 text-start font-semibold">{t("table.usage")}</th>
                  <th className="px-4 py-3 text-start font-semibold">{t("table.window")}</th>
                ...
                {rows.map((c) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <Link
                        href={`/coupons/${c.id}`}
                        className="text-foreground hover:text-primary font-mono text-sm font-semibold tracking-wide transition-colors"
                        dir="ltr"
                      >
                        {c.code}
                      </Link>
                      {c.description && (
                        <div className="text-muted-foreground mt-0.5 max-w-xs text-xs">{c.description}</div>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-foreground inline-flex items-center gap-1 font-medium tabular-nums">
                        {c.type === "PERCENTAGE" ? (
                          ...
                          formatSar(c.value, locale)
                        )}
                      </span>
                    </td>
                    <td className="text-foreground px-4 py-3 tabular-nums">
                      {c.minOrder ? formatSar(c.minOrder, locale) : <span className="text-muted-foreground">—</span>}
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-foreground tabular-nums">
                        {formatNumber(c.usageCount, locale)}
                        <span className="text-muted-foreground">
                          {" "}
                          / {c.usageLimit ? formatNumber(c.usageLimit, locale) : t("table.unlimited")}
                        </span>
                      </span>
                    </td>
